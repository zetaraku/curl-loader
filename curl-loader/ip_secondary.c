...
/*
  int
  set_alias_to_interface(const char*const alias_interface, const char*const ip_address, 
  const in_addr_t  mask, const in_addr_t bcast)
  {
  struct ifreq ifr;
  struct sockaddr_in sin;
  int sock = -1;

  if (!alias_interface || !strlen(alias_interface) || !ip_address || !strlen(ip_address))
  {
  fprintf (stderr, "set_alias_to_interface() - failed due to wrong input.\n");
  return -1;
  }
  if ((sock = socket (PF_INET, SOCK_DGRAM, 0)) == -1) 
  {
  fprintf (stderr, "set_alias_to_interface() - socket () call failed with errno %d.\n", errno);
  return -1;
  }

  // set the IP-address for the alias
  memset (&ifr, 0, sizeof (struct ifreq));
  strncpy (ifr.ifr_name, alias_interface, sizeof (ifr.ifr_name));
  ...
  {
  fprintf (stderr, "set_alias_to_interface() - ioctl() call failed for SIOCSIFADDR with errno %d.\n", errno);
  close (sock);
  return -1;
  }
  else
  fprintf (stderr, "Created alias interface \"%s\" with the address %s\n", alias_interface, ip_address);
    

  // set netmask for the alias interface
  //
  memset (&ifr, 0, sizeof (struct ifreq));
  strncpy (ifr.ifr_name, alias_interface, sizeof (ifr.ifr_name));
  memset (&sin, 0, sizeof (struct sockaddr_in));
  sin.sin_family = PF_INET;
  sin.sin_addr.s_addr = mask; // set the kept IP of the mask
  memcpy (&ifr.ifr_addr, &sin, sizeof (ifr.ifr_addr));

  if (ioctl (sock, SIOCSIFNETMASK, &ifr) == -1) 
  {
  fprintf (stderr, "set_alias_to_interface() - ioctl() call failed for SIOCSIFNETMASK with errno %d.\n", errno);
  close (sock);
  return -1;
  }
  else
  fprintf (stderr, "Netmask set to the alias interface \"%s\"\n", alias_interface);

  // set broadcast address for the alias interface
  //
  memset (&ifr, 0, sizeof (struct ifreq));
  strncpy (ifr.ifr_name, alias_interface, sizeof (ifr.ifr_name));
  memset (&sin, 0, sizeof (struct sockaddr_in));
  sin.sin_family = PF_INET;
  sin.sin_addr.s_addr = bcast;  // set the kept IP of the broadcast address
  memcpy (&ifr.ifr_broadaddr, &sin, sizeof (ifr.ifr_broadaddr));

  if (ioctl (sock, SIOCSIFBRDADDR, &ifr) == -1) 
  {
  fprintf (stderr, "set_alias_to_interface() - ioctl() call failed for SIOCSIFBRDADDR with errno %d.\n", errno);
  close (sock);
  return -1;
  }
  else
